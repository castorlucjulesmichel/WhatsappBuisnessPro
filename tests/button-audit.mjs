...
const recognized=/\b(data-page|data-go|data-setting-target|data-create-action|data-market-mode|data-wallet-action|data-chat-filter|data-info-msg)=/;
for(const m of html.matchAll(/<button\b([^>]*)>([\s\S]*?)<\/button>/g)){
  ...
  if(!id){errors.push("Button has no action/id: "+label);continue;}
  const refs=[`#${id}`,`"${id}"`,`'${id}'`];
  if(!refs.some(x=>js.includes(x)))errors.push("Button id has no JS handler/reference: "+id+" ("+label+")");
}
...
console.log("Button/UI audit passed: no duplicate IDs, broken route targets, dead explicit buttons, selector-forEach bugs, or JS syntax errors.");
