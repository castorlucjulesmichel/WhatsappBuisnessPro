...
if(configured()){
 ...
 onAuthStateChanged(auth,u=>{
   ...
   off=onSnapshot(query(collection(db,"notifications"),where("recipientId","==",u.uid)),s=>{
     ...
     if($("#notificationList")) $("#notificationList").innerHTML=rows.length?rows.map(n=>`<div class="notificationItem ${n.read===true?"":"unread"}" data-notification="${n.id}"><div>${esc(n.title||"Notifikasyon")}</div><small>${esc(n.message||"")}</small></div>`).join(""):'<p class="muted">Pa gen notifikasyon.</p>';
     $$("[data-notification]").forEach(el=>el.onclick=()=>updateDoc(doc(db,"notifications",el.dataset.notification),{read:true,readAt:serverTimestamp()}));
   });
 });
}
